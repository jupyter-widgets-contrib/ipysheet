...
describe('sheet', function () {
    ...
    let make_view = async function () {
        const view = await this.manager.create_view(this.sheet); // new ipysheet.SheetView({ model: this.sheet });
        await this.manager.display_view(undefined, view);
        return view;
    };

    let data_cloner = function () {
        let data = this.sheet.get('data');
        return JSON.parse(JSON.stringify(data));
    };

    let wait_validate = async function (view) {
    ...
    it('model reflecting view', async function () {
        let view = await make_view.call(this);
    ...
    it('view reflecting model', async function () {
        let view = await make_view.call(this);
        let data = this.sheet.get('data');
        let data_clone = data_cloner.call(this);
    ...
    it('view reflecting different view', async function () {
        let view1 = await make_view.call(this);
        let view2 = await make_view.call(this);
    ...
    let make_cell = async function (options, skip_add) {
        const modelId = 'u-u-i-d-cell';
        let cell = await this.manager.new_widget({
            model_module: 'ipysheet',
        ...
        let cells = this.sheet.get('cells');
        if (!skip_add) {
    ...
    let make_range = async function (options, skip_add) {
        const modelId = 'u-u-i-d-cell';
        let cell = await this.manager.new_widget({
            model_module: 'ipysheet',
            model_name: 'CellRangeModel',
    ...
    it('cell changes should be reflected in datamodel', async function () {
        let cell = await make_cell.apply(this, [{ value: 777 }]);
        let data = this.sheet.get('data');
        expect(data[1][2].value, 'for initial value').to.equal(777);
        cell.set('value', 999);
        data = this.sheet.get('data');
        expect(data[1][2].value, 'when cell.value is change').to.equal(999);
    });

    it('multiple cells added', async function () {
        let cell1 = await make_cell.apply(this, [{ value: 777 }, true]);
        let cell2 = await make_cell.apply(this, [{ row_start: 0, row_end: 0, value: 555 }, true]);
        let cells = this.sheet.get('cells');
    ...
    it('model changes should be reflected in cell', async function () {
        let cell = await make_cell.apply(this, [{ value: [[777]] }]);
        let data = data_cloner.call(this);
        data[1][2].value = 999;
    ...
    it('range should be reflected in all cells', async function () {
        let range = await make_range.apply(this, [{ row_start: 0, row_end: 1, column_start: 1, column_end: 2, value: [[0, 1], [2, 3]] }]);
        let data = data_cloner.call(this);
        ...
        await make_cell.apply(this, [{ row_start: 0, row_end: 0, value: 777 }]);
        ...
        let cell2 = await make_cell.apply(this, [{ row_start: 1, row_end: 1, value: null }]);
        data = data_cloner.call(this);
    ...
    it('range tranposed should be reflected in all cells', async function () {
        let range = await make_range.apply(this, [{ row_start: 0, row_end: 1, column_start: 1, column_end: 2, transpose: true, value: [[0, 1], [2, 3]] }]);
        let data = data_cloner.call(this);
        console.log(data[1][1]);
        expect(data[0][1].value, 'and in the underlying data grid').to.equal(0);
    ...
    it('should use the last style but not overwrite', async function () {
        let range = await make_range.apply(this, [{
            row_start: 0,
            row_end: 1,
            column_start: 1,
            column_end: 2,
        ...
        await make_cell.apply(this, [{ row_start: 0, row_end: 0, value: 777, style: { color: 'blue' } }]);
        data = data_cloner.call(this);
